...
Data Memory Access QSPI Controllerと FeRAM Access QSPI Controllerは、2つのメモリへのアクセスの切り替えを SPISSCTLビットで行います。
メモリへのアクセスを開始するときは、SPISSCTLを 0b01または 0b10に設定し、アクセス対象の SS信号をアサートします。
SPISSCTLを 0b11に設定することは禁止です。

Configuration Memory Access QSPI Controllerは、SS信号が 1本しか接続されておらず、OBCモジュールの基板上でセレクトされています。
そのため、Configurationデータ格納用 NOR Flash Memoryへアクセスを行う場合は、アクセスするメモリに関わらず SPISSCTLを 0b01にセットします。
Configurationデータ格納用 NOR Flash Memoryの メモリ選択は、System Registerの Configuration Memory Registerの設定により行うことができます。
