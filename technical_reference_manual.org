...
本レジスタにより Watchdog Counterの満了時間を設定する事ができます。
初期状態では、Watchdog Counterの満了時間は 128 [sec] に設定されています。
ソフトウェアが、定期的にSoftware Watchdog Timerをリロードできる状態となった後に、Software Watchdog Timeフィールドを適切な値に変更してください。
...
Software Watchdog Timeフィールドに設定されている間隔以内のタイミングで本レジスタに Writeアクセスを行い、Software Watchdog Timerをリロードしてください。
