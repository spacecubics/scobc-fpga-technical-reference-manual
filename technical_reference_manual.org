...
ECC Error Collect Enable Registerは、1bit ECCエラー検出時のメモリへの訂正データの書き戻し機能を設定するためのレジスタです。
...
本機能を有効にする場合は、訂正データの書き戻し機能(ECC Error Collect Enable RegisterのECCCOLEN)も合わせて有効にする必要があります。
