...
| 0x0008 | HRMEM_MEMSCRCTRLR    | Memory Scrubing Control Register            | 0x00010000 |
...
Memory Scrubing Control Registerは、メモリへのメモリスクラビング機能のON/OFF、および、メモリスクラビング機能制御に関する動作設定を行うためのレジスタです。
このレジスタのMemory Scrubing EnableをONにすると、メモリスクラビング機能による自動読み出しアクセスを行います。

メモリスクラビング機能を有効にする場合は、訂正データの書き戻し機能(ECC Error Collect Enable RegisterのECCCOLEN)も合わせて有効にする必要があります。

#+CAPTION: Memory Scrubing Control Register ビットフィールド
|   bit | Symbol      | Field                              | Description                                                                                                                                                   | R/W |
|-------+-------------+------------------------------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------+-----|
| 31:16 | MEMSCRCYC   | Memory Scrubing Cycle              | メモリスクラビングによる自動読み出しアクセスのWAIT Cycle間隔を設定する信号です。0: 設定禁止 1: 1Clock間隔(default) 2: 2Clock間隔 ... 65535: 65535Clock間隔    | R/W |
|  15:9 | -           | Reserved                           | Reserved                                                                                                                                                      | -   |
|     8 | COLFSRDSTPB | Memory Scrubing Arbitration Enable | 書き戻しデータ用FIFOがFULL時のメモリスクラビング調停機能を設定する信号です。本ビットが0の時、書き戻しデータ用FIFOがFULLの時にメモリスクラビングを停止します。 | R/W |
