...
| 0x0008 | HRMEM_MEMSCRCTRLR   | Memory Scrubing Control Register            | 0x06FF0000 |
...
| 0x0024 | HRMEM_ECC2ERRCNTR   | 2Bit ECC Error Count Register               | 0x00000000 |
...
|   bit | Symbol      | Field                              | Description                                                                                                                                                                                                | R/W |
|-------+-------------+------------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+-----|
| 31:16 | MEMSCRCYC   | Memory Scrubing Cycle              | メモリスクラビングによる自動読み出しアクセスのWAIT Cycle間隔を設定する信号です。0: 設定禁止 1: 1Clock間隔(default) 2: 2Clock間隔 ... 65535: 65535Clock間隔 初期状態では、1791Cycle間隔に設定されています。 | R/W |
...
1Bit ECC Error Count Registerは、1Bit ECCエラー検出件数を表示するカウンターレジスタです。1Bit ECCエラーの検出毎に本カウンターをインクリメントします。
...
|   bit | Symbol       | Field                                   | Description                                                                                          | R/W |
|-------+--------------+-----------------------------------------+------------------------------------------------------------------------------------------------------+-----|
| 31:16 | ATRDE1ERRCNT | Auto Read Access 1bit ECC Error Counter | メモリスクラビング機能の自動読み出しアクセス時の1bit ECCエラーをカウントする 16bitのカウンターです。 | RO  |
|  15:0 | AHBE1ERRCNT  | AHB Access 1bit ECC Error Counter       | AHBリードアクセス時の1bit ECCエラーをカウントする 16bitのカウンターです。                            | RO  |

*** 2Bit ECC Error Count Register (Offset: 0x0024)
2Bit ECC Error Count Registerは、2Bit ECCエラー検出件数を表示するカウンターレジスタです。2Bit ECCエラーの検出毎に本カウンターをインクリメントします。

カウンターが上限である 0xFFFFに達すると停止します。

#+CAPTION: 2Bit ECC Error Count Register ビットフィールド
|   bit | Symbol       | Field                                   | Description                                                                                          | R/W |
|-------+--------------+-----------------------------------------+------------------------------------------------------------------------------------------------------+-----|
| 31:16 | ATRDE2ERRCNT | Auto Read Access 2bit ECC Error Counter | メモリスクラビング機能の自動読み出しアクセス時の2bit ECCエラーをカウントする 16bitのカウンターです。 | RO  |
|  15:0 | AHBE2ERRCNT  | AHB Access 2bit ECC Error Counter       | AHBリードアクセス時の2bit ECCエラーをカウントする 16bitのカウンターです。                            | RO  |
...
最後にECC Errorを検出したアドレスが常に表示されます。
アドレスの値は、アクセス時の転送Byteのサイズに関わらず、Word(32Bit)アクセスの単位で表示されます。

#+CAPTION: ECC Error Address Monitor Register ビットフィールド
|   bit | Symbol    | Field             | Description                                                      | R/W |
|-------+-----------+-------------------+------------------------------------------------------------------+-----|
| 31:22 | -         | Reserved          | Reserved                                                         | -   |
|  21:0 | ECCERRADR | ECC Error Address | 1Bit ECC Errorおよび2Bit ECC Errorを検出したアドレスを示します。 | RO  |
